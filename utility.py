...
def Sampling(num_samples, time_all, time_pt, data_train, sigma, device):
    mu = data_train[time_all[time_pt]]
    num_gaussian, dim = mu.shape
    noise_dist = torch.distributions.MultivariateNormal(
        loc=torch.zeros(dim, device=device),
        covariance_matrix=sigma * torch.eye(dim, device=device)
    )
    noise_add = noise_dist.rsample((num_samples,))  # shape: (num_samples, dim)
    
    mu_tensor = torch.tensor(mu, device=device, dtype=noise_add.dtype)

    if num_gaussian < num_samples:
        idx = torch.randint(0, num_gaussian, (num_samples,), device=device)  # with replacement
    else:
        idx = torch.randperm(num_gaussian, device=device)[:num_samples]  # without replacement

    # Sample from mu and add noise
    sampled_mu = mu_tensor[idx]
